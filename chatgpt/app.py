...
def chat(
    message: str, state: List[Dict[str, str]], temperature: float, api_key: str, num_history: int
    ...
    if conv_history_tokens+max_response_tokens >= token_limit:
        ...
        response_message = ""
        try:
            num_history_process = num_history + 1
            # limit history to 3 messages
            if len(history_messages) == 2:
                history_messages_process = history_messages
            else:
                history_messages_process = history_messages[-num_history_process:]
