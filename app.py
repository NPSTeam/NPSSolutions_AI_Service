...
with block:
    ...
    with gr.Tab("Bóc băng Youtube"):
        ...
        with gr.Row().style(equal_height=True):   
          ...
          btn_send_summary.click(fn=lambda value: gr.update(value=value, lines=5), inputs=whisper_result, outputs=flag_textbox)
    ...
    with gr.Tab("Bóc băng Audio"):
        ...
        with gr.Row().style(equal_height=True):          
          ...
          btn_audio_send_summary.click(fn=lambda value: gr.update(value=value, lines=5), inputs=result_speech_to_text_with_file, outputs=flag_textbox)
    ...
    with gr.Tab("Hướng dẫn sử dụng"):
        ...
        with gr.Box(elem_id="custom_row"):
              ...
              message_docs = gr.Textbox(placeholder="Hỏi chatbot bất cứ vấn đề nào về NPSSolutions mà bạn muốn", label="Câu hỏi của bạn")
              state_docs = gr.State()
              with gr.Row().style(equal_height=True):
